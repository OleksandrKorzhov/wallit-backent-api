...
type NotificationQuery struct {
	...
	withNotificationRecipient     *UserQuery
	withNotificationDiscountOffer *DiscountOfferQuery
	withFKs                       bool
...
// QueryNotificationRecipient chains the current query on the "notification_recipient" edge.
func (nq *NotificationQuery) QueryNotificationRecipient() *UserQuery {
...
// QueryNotificationDiscountOffer chains the current query on the "notification_discount_offer" edge.
func (nq *NotificationQuery) QueryNotificationDiscountOffer() *DiscountOfferQuery {
	query := (&DiscountOfferClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withNotificationRecipient:     nq.withNotificationRecipient.Clone(),
		withNotificationDiscountOffer: nq.withNotificationDiscountOffer.Clone(),
...
// WithNotificationRecipient tells the query-builder to eager-load the nodes that are connected to
// the "notification_recipient" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithNotificationRecipient(opts ...func(*UserQuery)) *NotificationQuery {
	query := (&UserClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withNotificationRecipient = query
	return nq
}

// WithNotificationDiscountOffer tells the query-builder to eager-load the nodes that are connected to
// the "notification_discount_offer" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithNotificationDiscountOffer(opts ...func(*DiscountOfferQuery)) *NotificationQuery {
	query := (&DiscountOfferClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withNotificationDiscountOffer = query
	return nq
}
...
//		GroupBy(notification.FieldCheckedInApp).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Notification.Query().
//		Select(notification.FieldCheckedInApp).
//		Scan(ctx, &v)
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		_spec       = nq.querySpec()
		loadedTypes = [2]bool{
			nq.withNotificationRecipient != nil,
			nq.withNotificationDiscountOffer != nil,
		}
	)
	if nq.withNotificationRecipient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withNotificationRecipient; query != nil {
		if err := nq.loadNotificationRecipient(ctx, query, nodes, nil,
			func(n *Notification, e *User) { n.Edges.NotificationRecipient = e }); err != nil {
			return nil, err
		}
	}
	if query := nq.withNotificationDiscountOffer; query != nil {
		if err := nq.loadNotificationDiscountOffer(ctx, query, nodes, nil,
			func(n *Notification, e *DiscountOffer) { n.Edges.NotificationDiscountOffer = e }); err != nil {
			return nil, err
		}
	}
...
func (nq *NotificationQuery) loadNotificationRecipient(ctx context.Context, query *UserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *User)) error {
...
func (nq *NotificationQuery) loadNotificationDiscountOffer(ctx context.Context, query *DiscountOfferQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *DiscountOffer)) error {
	...
	query.withFKs = true
	query.Where(predicate.DiscountOffer(func(s *sql.Selector) {
		s.Where(sql.InValues(notification.NotificationDiscountOfferColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.notification_notification_discount_offer
		if fk == nil {
			return fmt.Errorf(`foreign-key "notification_notification_discount_offer" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_notification_discount_offer" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
