...
type TransactionQuery struct {
	...
	withInstitutionAccount         *PlaidInstitutionAccountQuery
	withTransactionCategories      *SpendingCategoryQuery
	withFKs                        bool
	modifiers                      []func(*sql.Selector)
	loadTotal                      []func(context.Context, []*Transaction) error
	withNamedTransactionCategories map[string]*SpendingCategoryQuery
...
// QueryInstitutionAccount chains the current query on the "institution_account" edge.
func (tq *TransactionQuery) QueryInstitutionAccount() *PlaidInstitutionAccountQuery {
	query := (&PlaidInstitutionAccountClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) QueryTransactionCategories() *SpendingCategoryQuery {
	query := (&SpendingCategoryClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withInstitutionAccount:    tq.withInstitutionAccount.Clone(),
		withTransactionCategories: tq.withTransactionCategories.Clone(),
...
// WithInstitutionAccount tells the query-builder to eager-load the nodes that are connected to
// the "institution_account" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithInstitutionAccount(opts ...func(*PlaidInstitutionAccountQuery)) *TransactionQuery {
	query := (&PlaidInstitutionAccountClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withInstitutionAccount = query
	return tq
}
...
func (tq *TransactionQuery) WithTransactionCategories(opts ...func(*SpendingCategoryQuery)) *TransactionQuery {
	query := (&SpendingCategoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTransactionCategories = query
	return tq
}
...
//		GroupBy(transaction.FieldFinancialAccountID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldFinancialAccountID).
//		Scan(ctx, &v)
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [2]bool{
			tq.withInstitutionAccount != nil,
			tq.withTransactionCategories != nil,
		}
	)
	if tq.withInstitutionAccount != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withInstitutionAccount; query != nil {
		if err := tq.loadInstitutionAccount(ctx, query, nodes, nil,
			func(n *Transaction, e *PlaidInstitutionAccount) { n.Edges.InstitutionAccount = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTransactionCategories; query != nil {
		if err := tq.loadTransactionCategories(ctx, query, nodes,
			func(n *Transaction) { n.Edges.TransactionCategories = []*SpendingCategory{} },
			func(n *Transaction, e *SpendingCategory) {
				n.Edges.TransactionCategories = append(n.Edges.TransactionCategories, e)
			}); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedTransactionCategories {
		if err := tq.loadTransactionCategories(ctx, query, nodes,
			func(n *Transaction) { n.appendNamedTransactionCategories(name) },
			func(n *Transaction, e *SpendingCategory) { n.appendNamedTransactionCategories(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TransactionQuery) loadInstitutionAccount(ctx context.Context, query *PlaidInstitutionAccountQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *PlaidInstitutionAccount)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].plaid_institution_account_transactions == nil {
			continue
		}
		fk := *nodes[i].plaid_institution_account_transactions
	...
	query.Where(plaidinstitutionaccount.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "plaid_institution_account_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadTransactionCategories(ctx context.Context, query *SpendingCategoryQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *SpendingCategory)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(transaction.TransactionCategoriesTable)
		s.Join(joinT).On(s.C(spendingcategory.FieldID), joinT.C(transaction.TransactionCategoriesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(transaction.TransactionCategoriesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(transaction.TransactionCategoriesPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "transaction_categories" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (tq *TransactionQuery) WithNamedTransactionCategories(name string, opts ...func(*SpendingCategoryQuery)) *TransactionQuery {
	query := (&SpendingCategoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedTransactionCategories == nil {
		tq.withNamedTransactionCategories = make(map[string]*SpendingCategoryQuery)
	}
	tq.withNamedTransactionCategories[name] = query
	return tq
}
