...
type MerchantQuery struct {
	...
	withDiscountOffers      *DiscountOfferQuery
	modifiers               []func(*sql.Selector)
	loadTotal               []func(context.Context, []*Merchant) error
	withNamedDiscountOffers map[string]*DiscountOfferQuery
...
// QueryDiscountOffers chains the current query on the "discount_offers" edge.
func (mq *MerchantQuery) QueryDiscountOffers() *DiscountOfferQuery {
	query := (&DiscountOfferClient{config: mq.config}).Query()
...
func (mq *MerchantQuery) Clone() *MerchantQuery {
	...
	return &MerchantQuery{
		...
		withDiscountOffers: mq.withDiscountOffers.Clone(),
...
// WithDiscountOffers tells the query-builder to eager-load the nodes that are connected to
// the "discount_offers" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MerchantQuery) WithDiscountOffers(opts ...func(*DiscountOfferQuery)) *MerchantQuery {
	query := (&DiscountOfferClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withDiscountOffers = query
	return mq
}
...
func (mq *MerchantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Merchant, error) {
	...
	if query := mq.withDiscountOffers; query != nil {
		if err := mq.loadDiscountOffers(ctx, query, nodes,
			func(n *Merchant) { n.Edges.DiscountOffers = []*DiscountOffer{} },
			func(n *Merchant, e *DiscountOffer) { n.Edges.DiscountOffers = append(n.Edges.DiscountOffers, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range mq.withNamedDiscountOffers {
		if err := mq.loadDiscountOffers(ctx, query, nodes,
			func(n *Merchant) { n.appendNamedDiscountOffers(name) },
			func(n *Merchant, e *DiscountOffer) { n.appendNamedDiscountOffers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MerchantQuery) loadDiscountOffers(ctx context.Context, query *DiscountOfferQuery, nodes []*Merchant, init func(*Merchant), assign func(*Merchant, *DiscountOffer)) error {
	...
	query.Where(predicate.DiscountOffer(func(s *sql.Selector) {
		s.Where(sql.InValues(merchant.DiscountOffersColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.merchant_discount_offers
		if fk == nil {
			return fmt.Errorf(`foreign-key "merchant_discount_offers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "merchant_discount_offers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedDiscountOffers tells the query-builder to eager-load the nodes that are connected to the "discount_offers"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (mq *MerchantQuery) WithNamedDiscountOffers(name string, opts ...func(*DiscountOfferQuery)) *MerchantQuery {
	query := (&DiscountOfferClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if mq.withNamedDiscountOffers == nil {
		mq.withNamedDiscountOffers = make(map[string]*DiscountOfferQuery)
	}
	mq.withNamedDiscountOffers[name] = query
	return mq
}
