...
func (tx *Tx) init() {
	tx.DiscountOffer = NewDiscountOfferClient(tx.config)
	tx.Merchant = NewMerchantClient(tx.config)
	tx.Notification = NewNotificationClient(tx.config)
	tx.PlaidInstitution = NewPlaidInstitutionClient(tx.config)
	tx.PlaidInstitutionAccount = NewPlaidInstitutionAccountClient(tx.config)
	tx.PlaidItem = NewPlaidItemClient(tx.config)
	tx.SpendingCategory = NewSpendingCategoryClient(tx.config)
	tx.Transaction = NewTransactionClient(tx.config)
	tx.TransactionSync = NewTransactionSyncClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.UserNotificationChannelPreferences = NewUserNotificationChannelPreferencesClient(tx.config)
}
...
// applies a query, for example: DiscountOffer.QueryXXX(), the query will be executed
