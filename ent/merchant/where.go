...
// HasDiscountOffers applies the HasEdge predicate on the "discount_offers" edge.
func HasDiscountOffers() predicate.Merchant {
	return predicate.Merchant(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DiscountOffersTable, DiscountOffersColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasDiscountOffersWith applies the HasEdge predicate on the "discount_offers" edge with a given conditions (other predicates).
func HasDiscountOffersWith(preds ...predicate.DiscountOffer) predicate.Merchant {
	return predicate.Merchant(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DiscountOffersInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DiscountOffersTable, DiscountOffersColumn),
		)
